...
@pytest.fixture(scope='session')
def ghf_data_water_sto3g(water_sto3g: ResultHF) -> GHF_Data:
...
def test_ccsd_energy(
    ghf_data_water_sto3g: GHF_Data,
    nuclear_repulsion_energy: float,
    ghf_ccsd_config: GHF_CCSD_Config,
) -> None:
    ccsd = GHF_CCSD(ghf_data_water_sto3g, ghf_ccsd_config)
...
def test_lambda_solver(ghf_data_water_sto3g: GHF_Data) -> None:
    ccsd = GHF_CCSD(ghf_data_water_sto3g, config=GHF_CCSD_Config(verbose=True))
